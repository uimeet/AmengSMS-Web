...
(function (define) {
    ...
    define([
        ...
        function cometService($state, $timeout, growl, settings, authentication) {
             ...
             * {
             *      // 频道
             *      channel: 'bn_' + userInfo.id,
             *      // 服务器推送后的回调函数
                    callback: this.onCometCallback,
                    // 获取token的地址
                    signUrl: settings.api_prefix + '/comet-make-token',
                    // 订阅地址
                    subUrl: 'http://' + settings.comet.host + (settings.comet.port == 80 ? '' : ':' + settings.comet.port) + '/ic/sub',
                    // 是否开启调试
                    debug: settings.debug,
                    // 签名地址所需的授权key
                    authorized_key: userInfo.authorized_key
             * }
             * */
            var iComet = function (config) {
                var self = this;
                ...
                self.cb_name = 'icomet_cb_' + self.id;
                self.timer = null;
                ...
                if (config.sign_url) {
                    ...
                    self.sign_url += 'cb=' + self.cb_name + '&cname=' + self.cname;
                }

                window[self.cb_name] = function (msg, in_batch) {
                    // batch repsonse
                    if (msg instanceof Array) {
                        ...
                        for (var i in msg) {
                            if (msg[i] && msg[i].type == 'data') {
                                if (i == msg.length - 1) {
                                    window[self.cb_name](msg[i]);
                                } else {
                                    window[self.cb_name](msg[i], true);
                                }
                            }
                        }
                        return;
                    ...
                    if (msg.type == 'noauth') {
                        self.log('resp', msg);
                        if (self.config.noAuthCallback) {
                            self.config.noAuthCallback();
                        }
                        return;
                    }
                    if (msg.type == 'noop') {
                ...
                self.sign = function (callback) {
                    ...
                    var url = self.sign_url + '&authorized_key=' + self.config.authorized_key + '&_=' + new Date().getTime();
                    $.ajax({
                        url: url,
                        dataType: "jsonp",
                        jsonpCallback: "cb"
            ...
            this.on = function (eventName, callback) {
                ...
                listeners[eventName][callback.toString()] = callback;
                return self;
            };
            /**
             * 移除给定事件的监听器
            ...
            this.off = function (eventName, callback) {
                ...
                if (listeners[eventName] && listeners[eventName][key]) {
            ...
            this.setup = function (force) {
                if (settings.comet && settings.comet.enabled) {
                    if (!self.comet || force) {
                        var userInfo = authentication.getUserInfo();
                        if (userInfo) {
                            host = settings.comet.host;
                            port = settings.comet.port;
                            self.comet = new iComet({
                                channel: 'bn_' + userInfo.hash_id,
                                callback: this.onCometCallback,
                                noAuthCallback: this.onOnAuthCallback,
                                signUrl: settings.api_prefix + '/comet/token',
                                subUrl: 'http://' + host + (port == 80 ? '' : ':' + port) + '/ic/sub',
                                debug: settings.debug,
                                authorized_key: userInfo.authorized_key
                            });
                            // 触发启动事件
                            self.dispatchEventAsync(self.EVENTS.Setup, self);
                        } else {
                            console.log('Unauthorized user in cometService.');
                        }
                    }
                } else {
                    console.warn('settings.comet not found or disabled.');
                }
                return self;
            };
            this.onOnAuthCallback = function () {
                growl.error('登录超时，或账号在别处登录');
                self.stop();
                $state.go('front.home');
            };
            this.onCometCallback = function (data) {
                data = JSON.parse(data);
                return self.dispatchEventAsync(data.type, data.data);
            };
            this.stop = function () {
                if (self.comet) {
                    self.comet.stop();
                ...
                // 触发关闭事件
                self.dispatchEventAsync(self.EVENTS.Close, self);
            };
            /**
             * 触发给定事件
             * */
            this.dispatchEvent = function (eventName, params) {
                if (listeners[eventName]) {
                    for (var uniqueId in listeners[eventName]) {
                        listeners[eventName][uniqueId](params);
                    }
                }
                return self;
            };
            this.dispatchEventAsync = function (eventName, params) {
                var self = this;
                $timeout(function () {
                    self.dispatchEvent(eventName, params);
                });
                return self;
            };
        ...
        angular.module('amengsms.services.comet', [
            'ui.router',
            'angular-growl',
            'amengsms.settings',
            'amengsms.services.authentication'
        ])
        .service('iComet', ['$state', '$timeout', 'growl', 'settings', 'authentication', cometService]);
